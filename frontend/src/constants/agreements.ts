export const PERSONAL_AGREEMENT = `**개인정보처리방침**

제1조(목적)

이 약관은 Hikingdom (이하 '회사' 라고 합니다)가 제공하는 제반 서비스의 이용과 관련하여 회사와 회원과의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
...
3. 회사가 제공하는 서비스의 내용은 다음과 같습니다.
    1. 산정보, 하이킹 기록 정보, 등반모임 생성, 하이킹 기록 저장
    2. 모임 일정 공유, 모임원들간의 채팅, 모임원들과의 위치공유
...
본 약관은 2023.05.10부터 시행됩니다.


**개인정보처리동의서**

Hikingdom(이하 '회사'라고 합니다)은(는) 개인정보보호법 등 관련 법령상의 개인정보보호 규정을 준수하며 귀하의 개인정보보호에 최선을 다하고 있습니다. 회사는 개인정보보호법에 근거하여 다음과 같은 내용으로 개인정보를 수집 및 처리하고자 합니다.
...
export const USAGE_AGREEMENT = `**서비스 이용약관**
...
export const LOCATION_AGREEMENT = `**위치기반서비스 이용약관**

제1조(목적)

본 약관은 회원(Hikingdom의 서비스 약관에 동의한 자를 말하며 이하 '회원'이라고 합니다)이 Hikingdom(이하 '회사'라고 합니다)가 제공하는 웹페이지 및 'hikingdom' (회사가 개발 운영하는 모바일 애플리케이션을 말합니다 이하 '모바일앱'이라고 합니다)의 서비스를 이용함에 있어 회원과 회사의 권리 및 의무, 기타 제반 사항을 정하는 것을 목적으로 합니다.
...
5. 회사가 약관을 개정할 경우 기존약관과 개정약관 및 개정약관의 적용일자와 개정사유를 명시하여 현행약관과 함께 그 적용일자 10일 전부터 적용일 이후 상당한 기간 동안 회사의 웹페이지 및 모바일앱을 통해 공지합니다. 다만, 개정약관의 내용이 회원에게 새로운 의무를 부과하거나 권리를 제한하는 내용인 경우 그 적용일자 30일 전부터 상당한 기간 동안 이를 회사의 웹페이지 및 모바일앱을 통해 공지하고, 회원에게 전자적형태로 약관의 개정사실을 발송하여 고지합니다.
...
1. 길 안내 등 생활편의 서비스 제공
2. 등반을 같이 하는 모임원들끼리의 위치공유 서비스 제공

제8조(서비스 이용요금)

1. 회사의 서비스는 무료제공을 원칙으로 합니다.
2. 전항에도 불구하고 무선 서비스 이용시 발생하는 데이터 통신료는 별도이며, 이 때 부과되는 데이터 통신료는 회원이 가입한 각 이동통신사의 정책에 따릅니다.
...
1. 회사는 아래 각 호에 해당하는 사유가 발생한 경우에는 회원의 서비스 이용을 제한하거나 중지시킬 수 있습니다.
    ...
    3. 전기통신사업법에 규정된 기간통신사업자가 서비스를 중지했을 경우
    4. 국가비상사태, 서비스 설비의 장애 또는 서비스 이용의 폭주 등으로 서비스 이용에 지장이 있는 때
    5. 기타 중대한 사유로 회사가 서비스 제공을 지속하는 것이 곤란한 경우
...
3. 회사는 타사업자 또는 이용 고객의 민원처리 등을 위해 회원의 위치정보 이용 · 제공사실 확인자료를 자동 기록 · 보존하며, 해당 자료는 1일간 보관합니다.
4. 회사가 개인위치정보를 회원이 지정하는 제3자에게 제공하는 경우, 위치정보를 수집한 당해 통신 단말장치로 매회 회원에게 제공받는 자, 제공일시, 제공목적을 즉시 통보합니다. 다만 다음 각호의 경우에는 회원이 미리 지정한 통신단말장치 또는 전자우편주소로 통보합니다.
    1. 개인위치정보를 수집한 통신단말장치가 문자, 음성 또는 영상 수신기능을 갖추지 아니한 경우
    2. 회원이 다른 방법을 요청한 경우

제12조(개인위치정보의 보유기간 및 이용기간)

1. 회사가 회원의 개인위치정보를 이용하였다면 회사는 위치정보의 보호 및 이용 등에 관한 법률 제16조 제2항의 규정에 따라 기록 · 보존해야 하는 위치정보이용 제공사실 확인자료 이외의 해당 개인위치정보를 같은 법 제23조에 따라 고객이 동의한 목적범위 내에서 이용하고 고객 응대를 위하여 1일간 보유하며, 이 기간이 지나면 즉시 파기합니다.
...
2. 회사는 14세 미만 회원의 개인위치정보 및 위치정보 이용, 제공사실에 관한 확인자료를 본 약관에 명시 또는 고지한 범위를 넘어 이용하거나, 제3자에게 제공하고자 하는 경우 당해 회원과 회원의 법정대리인에게 모두 동의를 받아야합니다. 다만 다음 각호의 경우는 제외합니다.
    1. 개인위치정보 및 위치기반서비스 제공에 따른 요금정산을 위하여 위치정보 이용, 제공사실 확인자료가 필요한 경우
    2. 통계작성, 학술연구 또는 시장조사를 위하여 특정 개인을 알아볼 수 없는 형태로 가공하여 제공하는 경우
    3. 기타 관계법령에 특별한 규정이 있는 경우

제15조(위치정보관리책임자의 지정)

1. 회사는 위치정보를 적절히 관리, 보호하고 개인위치정보주체의 불만을 원활히 처리할 수 있도록 실질적인 책임을 질 수 있는 지위에 있는 자를 위치정보의 관리책임자로 지정하고 운영합니다.
2. 회사의 위치정보관리책임자는 위치기반서비스의 제공에 관한 제반 사항을 담당 · 관리하는 부서의 CSO으로서, 구체적인 사항은 본 약관의 부칙에 따릅니다.
...
3. 제1항 내지 제2항에도 불구하고 당사자간 분쟁이 원만하게 해결되지 않아 법원에 의한 분쟁해결 절차를 따를 경우에는 서울중앙지방법원을 전속적인 제1심 관할법원으로 정합니다.
...
1. 법인명 : Hikingdom
2. 대표이사 : 이병호
3. 소재지 : 서울 강남구 테헤란로 212 (역삼동, 멀티캠퍼스)
4. 연락처 : lightnsalt@gmail.com

부 칙

제1조(시행일)

본 약관은 2023.05.10부터 시행합니다.

제2조(위치정보관리책임자)

위치정보관리책임자는 2023.05.01를 기준으로 다음과 같이 지정합니다.

1. 소속 : Hikingdom
2. 성명 : 이병호
3. 직위 : CSO
4. 전화 : lightnsalt@gmail.com`
